        ...
        {typeof capacity === 'number' && capacity > 0 && (
          <p className="mb-4 flex flex-wrap items-baseline gap-x-2 gap-y-1">
            ...
            {counts.attending >= capacity && (
              <span className="rounded-pill bg-warn-soft px-2.5 py-0.5 font-sans text-[11px] uppercase tracking-[0.1em] text-warn-text">Full</span>
            )}
          </p>
        )}
  ...
  const inner = (
    <>
      <div className="min-w-0">
        ...
        {sub && <p className="font-sans text-[12px] uppercase tracking-[0.14em] text-ink-3">{sub}</p>}
      </div>
      <div className="flex items-center gap-3">
        {trailing && <span className="text-sm text-ink-2">{trailing}</span>}
        {pill && (
          <span className="rounded-pill bg-paper-2 px-2.5 py-1 font-sans text-[11px] uppercase tracking-[0.14em] text-ink-3">{pill}</span>
        )}
      </div>
    </>
  )
